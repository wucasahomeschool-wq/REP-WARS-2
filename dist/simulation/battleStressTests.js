...
exports.formatBatchStats = exports.RATIO_SCENARIOS = exports.EQUAL_FORCE_SCENARIOS = exports.EXTREME_RATIO_SCENARIOS = void 0;
exports.runStressScenario = runStressScenario;
exports.assertAttackerTroopMonotonicity = assertAttackerTroopMonotonicity;
exports.assertAttackerMoraleMonotonicity = assertAttackerMoraleMonotonicity;
exports.assertDefenseBonusMonotonicity = assertDefenseBonusMonotonicity;
exports.runFullStressSuite = runFullStressSuite;
exports.formatStressCsvReport = formatStressCsvReport;
...
function plainTroops(atk, def, terrain = 'plains', atkMorale = 75, defMorale = 75, atkQuality = 1.0, defQuality = 1.0, fortification = 0) {
    return {
        attackerTroops: atk,
...
exports.EXTREME_RATIO_SCENARIOS = [
    ...
    { label: '1,000,000 vs 10 (plains)', params: plainTroops(1000000, 10), expected: { minAttackerWinRate: 0.999, maxAttackerCasRate: 0.02, minDefenderCasRate: 0.9 } },
    { label: '10,000,000 vs 10 (plains)', params: plainTroops(10000000, 10), expected: { minAttackerWinRate: 0.999, maxAttackerCasRate: 0.015, minDefenderCasRate: 0.92 } },
    { label: '100,000,000 vs 10 (plains)', params: plainTroops(100000000, 10), expected: { minAttackerWinRate: 0.999, maxAttackerCasRate: 0.012, minDefenderCasRate: 0.95 } },
];
exports.EQUAL_FORCE_SCENARIOS = [
...
function runStressScenario(scenario, runs = 1000, seedBase = 500000) {
    const stats = (0, battleTests_1.runBattleLabBatch)(scenario.params, runs, seedBase);
    const checks = [];
...
function assertAttackerTroopMonotonicity(runsPerStep = 2000, seedBase = 700000) {
    ...
    for (let idx = 0; idx < steps.length; idx++) {
        ...
        for (let i = 0; i < runsPerStep; i++) {
            const input = (0, battleTests_1.buildLabInput)(plainTroops(atk, 100), seedBase + idx * 10000 + i);
...
function assertAttackerMoraleMonotonicity(runsPerStep = 2000, seedBase = 720000) {
    ...
    for (let idx = 0; idx < moraleSteps.length; idx++) {
        ...
        let wins = 0;
        for (let i = 0; i < runsPerStep; i++) {
            const input = (0, battleTests_1.buildLabInput)({
...
function assertDefenseBonusMonotonicity(runsPerStep = 2000, seedBase = 740000) {
    const terrains = [
...
function runFullStressSuite(runsPerScenario = 1000) {
    const extremeReports = exports.EXTREME_RATIO_SCENARIOS.map((s, i) => runStressScenario(s, runsPerScenario, 500000 + i * 20000));
    const equalReports = exports.EQUAL_FORCE_SCENARIOS.map((s, i) => runStressScenario(s, runsPerScenario, 600000 + i * 20000));
    const ratioReports = exports.RATIO_SCENARIOS.map((s, i) => runStressScenario(s, runsPerScenario, 650000 + i * 20000));
    const properties = [
...
function formatStressCsvReport(suite) {
    const rows = [];
...
//# sourceMappingURL=battleStressTests.js.map