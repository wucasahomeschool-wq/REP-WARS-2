...
exports.scenario_1_StableEmpire = scenario_1_StableEmpire;
exports.scenario_2_ForceDrought = scenario_2_ForceDrought;
exports.scenario_3_DroughtChainInterrupted = scenario_3_DroughtChainInterrupted;
exports.scenario_4_UnrestToRebellion = scenario_4_UnrestToRebellion;
exports.scenario_5_EventExpires = scenario_5_EventExpires;
exports.scenario_6_MultipleSimultaneousEvents = scenario_6_MultipleSimultaneousEvents;
exports.scenario_7_PositiveEvents = scenario_7_PositiveEvents;
exports.scenario_8_DeterministicReproducibility = scenario_8_DeterministicReproducibility;
exports.scenario_9_HistoryRecording = scenario_9_HistoryRecording;
exports.scenario_10_RegionalDifferences = scenario_10_RegionalDifferences;
exports.createInitialEmpire = createInitialEmpire;
exports.reportFactionStatus = reportFactionStatus;
...
function createInitialEmpire(seed, playerFactionId = 'merchant_republic') {
    const { gameState } = index_1.SimulationBuilder.buildFromSpecs(index_1.SAMPLE_MAP, index_1.WARLORD_SPECS, seed);
    return {
        gameState,
        activeEvents: [],
        eventHistory: [],
...
function reportFactionStatus(st) {
...
function printTurnOutput(st, out) {
    console.log(`\n\nTURN ${st.turn}`);
    hr();
    ...
    for (const te of out.triggeredEvents) {
        ...
        const inst = out.newActiveEvents.find(e => e.typeId === te.definition.typeId && e.territoryId === te.territoryId);
        if (!inst)
            continue;
        hrThin();
        ...
        if (inst.causes.length > 0)
            console.log(`  Causes: ${inst.causes.filter((_, i) => i < 3).join(' • ')}`);
        ...
        if (te.immediateConsequences.length > 0) {
            ...
            for (const c of te.immediateConsequences) {
                const d = c.delta;
                const parts = [];
                if (d.stabilityDelta !== undefined)
                    parts.push(`Stability ${d.stabilityDelta > 0 ? '+' : ''}${d.stabilityDelta}`);
                if (d.foodProductionPct !== undefined)
                    parts.push(`Food prod ${Math.round(d.foodProductionPct * 100)}%`);
                if (d.garrisonDeltaAbs !== undefined)
                    parts.push(`Garrison ${d.garrisonDeltaAbs > 0 ? '+' : ''}${d.garrisonDeltaAbs}`);
                if (d.populationDeltaAbs !== undefined)
                    parts.push(`Population ${d.populationDeltaAbs > 0 ? '+' : ''}${d.populationDeltaAbs.toLocaleString()}`);
                if (d.resources) {
                ...
                if (d.resourceOutputPct)
                    parts.push(`Resource output boost`);
...
function printHistory(st, limitPerTurn = 3) {
    console.log('\n─── EVENT HISTORY ───');
    const grouped = {};
...
// ────────────────────────────────────────────
function scenario_5_EventExpires(seed) {
    console.log('\n\n\n╔══════════════════════════════════════════════════════════════════════╗');
    ...
    for (let step = 0; step < 8; step++) {
        ...
        if (out.expiredEvents.length > 0) {
            ...
            for (const ex of out.expiredEvents)
                console.log(`      - ${ex.title}  (lasted T${ex.startedTurn}–T${ex.expiresTurn - 1})`);
        }
        st.gameState.territories = out.mutatedTerritories;
        st.gameState.factions = out.mutatedFactions;
        st.activeEvents = out.newActiveEvents;
...
// ────────────────────────────────────────────
function scenario_7_PositiveEvents(seed) {
    console.log('\n\n\n╔══════════════════════════════════════════════════════════════════════╗');
    ...
    for (let i = 0; i < 5; i++) {
        ...
        if (out.summary.length > 0)
            printTurnOutput(st, out);
        const positive = out.triggeredEvents.filter(te => te.definition.typeId === 'prosperity' || te.definition.typeId === 'trade_boom' || te.definition.typeId === 'resource_discovery');
...
function scenario_8_DeterministicReproducibility() {
    ...
    function runOnce(s) {
        ...
        for (let i = 0; i < 6; i++) {
            ...
            const out = sim.simulate(input);
            for (const e of out.triggeredEvents)
                events.push(`T${st.turn}:${e.definition.typeId}:${e.territoryId}:${e.severity}`);
            st.gameState.territories = out.mutatedTerritories;
            st.gameState.factions = out.mutatedFactions;
            st.activeEvents = out.newActiveEvents;
            st.eventHistory = out.newEventHistory;
    ...
    for (const k of r1.stab.keys()) {
        if (r1.stab.get(k) !== r2.stab.get(k))
            stabsEqual = false;
    }
...
function scenario_10_RegionalDifferences() {
    ...
    const sim = new index_1.WorldSimulator();
    const rng = new index_1.SeededRNG(4242);
    console.log('Evaluating trigger weights for representative territories:');
    console.log();
    const { gameState } = index_1.SimulationBuilder.buildFromSpecs(index_1.SAMPLE_MAP, index_1.WARLORD_SPECS, 42);
    for (const c of cases) {
        ...
        const helper = new index_1.WorldHelperImpl(gameState.territories, gameState.factions, []);
        const ctx = (0, index_1.buildConditionContext)(10, c.tid, 'merchant_republic', gameState.territories, gameState.factions, [], rng.fork(c.tid.length));
        const results = (0, index_1.evaluateAllTriggersForTerritory)(ctx);
        t.owner = prevOwner;
        ...
        for (const r of tops) {
            ...
            if (r.reasons.length)
                console.log(`        reasons: ${r.reasons.slice(0, 2).join('; ')}`);
        }
        console.log();
    }
...
function main() {
    ...
    console.log('Balance config:', JSON.stringify({
        maxEventsPerTurn: index_1.BALANCE.events.world.maxEventsPerTurn,
        severities: Object.keys(index_1.BALANCE.events.severityWeights).length,
        triggers: Object.keys(index_1.BALANCE.events.trigger).length,
        chains: Object.keys(index_1.BALANCE.events.chain).length,
    }, null, 2));
    const seed = 42;
    try {
        scenario_1_StableEmpire(seed);
    }
    catch (e) {
    ...
    try {
        scenario_2_ForceDrought();
    }
    catch (e) {
        console.error('✗ Scenario 2 crashed:', e);
        process.exit(1);
    }
    try {
        scenario_3_DroughtChainInterrupted();
    }
    catch (e) {
    ...
    try {
        ...
        console.error('✗ Scenario 4 crashed:', e);
        process.exit(1);
    }
    try {
        scenario_5_EventExpires(808);
    }
    catch (e) {
        console.error('✗ Scenario 5 crashed:', e);
        process.exit(1);
    }
    try {
        ...
        console.error('✗ Scenario 6 crashed:', e);
        process.exit(1);
    }
    try {
        scenario_7_PositiveEvents(303);
    }
    catch (e) {
        console.error('✗ Scenario 7 crashed:', e);
        process.exit(1);
    }
    const det = scenario_8_DeterministicReproducibility();
    try {
        scenario_9_HistoryRecording();
    }
    catch (e) {
        console.error('✗ Scenario 9 crashed:', e);
        process.exit(1);
    }
    try {
        scenario_10_RegionalDifferences();
    }
    catch (e) {
        console.error('✗ Scenario 10 crashed:', e);
        process.exit(1);
    }
    hr();
