...
import { ACTION_NAMES } from '../constants/balance';

export class WarlordState {
  public snapshot: WarlordSnapshot;
  ...
  static buildContext(
    ...
    const myNeighborIds = new Set<TerritoryId>();
    for (const t of myTerritories) {
      for (const nId of t.neighboring) myNeighborIds.add(nId);
    }
    ...
    const enemyNeighbors = myNeighboringTerritories.filter((t) => {
      if (!t.owner || t.owner === self.id) return false;
    ...
    for (const id of self.knownFactions) {
      ...
      if (!rel) { knownNeutrals.push(id); continue; }
      if (rel.state === 'at_war' || rel.state === 'hostile') knownEnemies.push(id);
      else if (rel.state === 'allied' || rel.state === 'friendly') knownAllies.push(id);
      else knownNeutrals.push(id);
    }
...
export class DecisionEngine {
  private scorer = new ActionScorer();
  ...
  private selectWithRandomness(
    scored: ScoredAction[],
    rng: SeededRNG
  ): { selected: ScoredAction; alternatives: ScoredAction[]; confidence: number } {
  ...
  formatDecision(decision: Decision, verbose: boolean = false): string {
  ...
  formatTurnReport(turn: number, decisions: Decision[], verbose: boolean = false): string {
    const lines: string[] = [];
    lines.push('');
...
function hashFactionId(id: FactionId): number {
  let h = 0;
