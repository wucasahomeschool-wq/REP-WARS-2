...
type PersonalityTraits = Omit<Personality, 'type'>;
type TraitKey = keyof PersonalityTraits;

const PERSONALITY_PRESETS: Record<PersonalityType, PersonalityTraits> = {
  defensive: {
    aggression: 0.2,
    defensiveness: 0.95,
    expansionism: 0.25,
...
const ACTION_PERSONALITY_BIAS: Record<ActionType, TraitKey> = {
  ATTACK: 'aggression',
...
export class PersonalitySystem {
  ...
  static randomizePreset(base: PersonalityType, variance = 0.15, rng: { next: () => number }): Personality {
    ...
    const result: Personality = { type: base, ...preset };
    for (const key of Object.keys(preset) as TraitKey[]) {
