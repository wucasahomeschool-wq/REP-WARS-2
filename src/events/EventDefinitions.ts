...
import {
  EventDefinition, ConditionContext, ConsequenceDelta, EventSeverity,
  ImperialChoice, buildDefaultSeverityTable,
  pickSeverityFromTable, severityScale, severityAtLeast,
} from './EventModel';
import { buildTriggerFn, COMMON_ELIGIBILITY } from './EventTriggers';

const B = BALANCE.events;
const BT = B.trigger;
...
function scale(sev: EventSeverity, base: number): number {
  return Math.round(base * severityScale(sev));
}

// ────────────────────────────────────────────
// 1. DROUGHT (environmental)
// ────────────────────────────────────────────
const DROUGHT: EventDefinition = {
  id: 'evt_drought',
  typeId: 'drought',
  ...
  descriptionTemplate: (sev, ctx) => {
    ...
    if (sev === 'critical') return loc + ' suffers catastrophic drought — reservoirs empty and agriculture collapses.';
    if (sev === 'severe') return 'A severe drought parches ' + loc + ', threatening harvests and water supplies.';
    if (sev === 'moderate') return 'Rainfall has been unusually low in ' + loc + '. Crops wither, rivers shrink, and the land grows parched.';
  ...
  severityTable: buildDefaultSeverityTable(),
  ...
  onTrigger: (ctx, sev) => {
    ...
    const scaleVal = severityScale(sev);
...
const FLOOD: EventDefinition = {
  ...
  descriptionTemplate: (sev, ctx) => {
    const loc = terrName(ctx);
    if (sev === 'critical') return 'Catastrophic floodwaters have submerged ' + loc + '; entire villages are swept away.';
    if (sev === 'severe') return 'Severe floods batter ' + loc + '; rivers burst their banks and damage settlements.';
    if (sev === 'moderate') return 'Heavy rains have caused moderate flooding across ' + loc + '.';
...
const STORM: EventDefinition = {
  ...
  descriptionTemplate: (sev, ctx) => {
    const loc = terrName(ctx);
    if (sev === 'critical') return 'A hurricane-force storm strikes ' + loc + '; roofs are torn off and coastal flooding is widespread.';
    if (sev === 'severe') return 'A severe storm batters ' + loc + '; trade and military readiness decline.';
    if (sev === 'moderate') return 'A strong storm sweeps through ' + loc + '.';
...
// ────────────────────────────────────────────
const HARSH_WINTER: EventDefinition = {
  id: 'evt_harsh_winter',
  typeId: 'harsh_winter',
  category: 'environmental',
  categoryLabel: 'Environmental',
  ...
  descriptionTemplate: (sev, ctx) => {
    const loc = terrName(ctx);
    if (sev === 'critical') return 'The worst winter in a generation freezes ' + loc + ' deep; roads close and populations suffer.';
    if (sev === 'severe') return 'A brutal winter descends on ' + loc + '; supplies dwindle and unrest simmers.';
    if (sev === 'moderate') return 'A harsh winter sets in across ' + loc + '.';
...
// ────────────────────────────────────────────
const RESOURCE_DISCOVERY: EventDefinition = {
  ...
  descriptionTemplate: (sev, ctx) => {
    const loc = terrName(ctx);
    if (sev === 'critical') return 'An extraordinary new lode of resources is struck in ' + loc + ' — prospectors flock!';
    if (sev === 'severe') return 'Prospectors in ' + loc + ' uncover a rich new resource deposit.';
    if (sev === 'moderate') return 'A useful new resource vein is found in ' + loc + '.';
...
// ────────────────────────────────────────────
const FOOD_SHORTAGE: EventDefinition = {
  ...
  descriptionTemplate: (sev, ctx) => {
    const loc = terrName(ctx);
    if (sev === 'critical') return loc + ' faces imminent famine without immediate intervention.';
    if (sev === 'severe') return 'Food supplies in ' + loc + ' are running dangerously short; prices skyrocket.';
    if (sev === 'moderate') return loc + ' is experiencing a food shortage.';
...
const FAMINE: EventDefinition = {
  ...
  descriptionTemplate: (sev, ctx) => {
    const loc = terrName(ctx);
    if (sev === 'critical') return loc + ' is ravaged by catastrophic famine — the dead litter the streets.';
    if (sev === 'severe') return 'Famine tightens its grip on ' + loc + '; refugees flee by the thousand.';
    if (sev === 'moderate') return 'Famine spreads in ' + loc + '.';
...
// ────────────────────────────────────────────
const PROSPERITY: EventDefinition = {
  id: 'evt_prosperity',
  typeId: 'prosperity',
  category: 'economic',
  ...
  descriptionTemplate: (sev, ctx) => {
    const loc = terrName(ctx);
    if (sev === 'critical') return loc + ' enters a golden age — wealth and plenty abound.';
    if (sev === 'severe') return 'A prosperous time for ' + loc + '; markets boom and people thrive.';
    if (sev === 'moderate') return loc + ' enjoys growing prosperity.';
...
// ────────────────────────────────────────────
const TRADE_BOOM: EventDefinition = {
  id: 'evt_trade_boom',
  typeId: 'trade_boom',
  category: 'economic',
  categoryLabel: 'Economic (Boons)',
  ...
  descriptionTemplate: (sev, ctx) => {
    const loc = terrName(ctx);
    if (sev === 'critical') return 'Caravans and ships flood into ' + loc + ' — unprecedented commerce!';
    if (sev === 'severe') return 'Traders flood into ' + loc + '; trade surges across all routes.';
    if (sev === 'moderate') return 'Trade booms in ' + loc + '.';
  ...
  onTrigger: (ctx, sev) => {
    ...
    return {
      ...
      consequences: [
        ...
        { territoryId: loc, delta: { resourceOutputPct: { gold: 0.15 * severityScale(sev) } }, message: 'Merchant quarter expands' },
      ],
    };
  },
};
...
const UNREST: EventDefinition = {
  ...
  descriptionTemplate: (sev, ctx) => {
    const loc = terrName(ctx);
    if (sev === 'critical') return loc + ' is on the brink of open rebellion!';
    if (sev === 'severe') return 'Widespread unrest sweeps ' + loc + '; crowds riot in the streets.';
    if (sev === 'moderate') return 'Protests and unrest in ' + loc + '.';
  ...
  choices: (_ctx, sev) => {
    const costGold = Math.round(BH.sendTroopsGarrisonCostPerSeverity * severityScale(sev));
    const goldPenalty = BH.reduceTaxesGoldPenaltyPerSeverity[sev];
    return [
  ...
  resolveChoice: (choiceId, ctx, sev, active) => {
    ...
    switch (choiceId) {
      case 'send_troops': {
        ...
        const stabRestore = Math.round(BH.sendTroopsStabilityRestorePerSeverity * severityScale(sev));
...
// ────────────────────────────────────────────
const REBELLION: EventDefinition = {
  id: 'evt_rebellion',
  typeId: 'rebellion',
  category: 'political',
  ...
  descriptionTemplate: (sev, ctx) => {
    const loc = terrName(ctx);
    if (sev === 'critical') return loc + ' erupts in full-scale rebellion! The garrison is besieged!';
    if (sev === 'severe') return 'Open rebellion in ' + loc + '; rebels seize parts of the countryside.';
    if (sev === 'moderate') return 'Armed rebellion breaks out in ' + loc + '.';
  ...
  severityTable: buildDefaultSeverityTable(),
  choices: (_ctx, sev) => {
    const costGold = Math.round(BH.sendTroopsGarrisonCostPerSeverity * severityScale(sev) * 2);
...
// ────────────────────────────────────────────
const BORDER_TENSION: EventDefinition = {
  ...
  descriptionTemplate: (sev, ctx) => {
    const loc = terrName(ctx);
    if (sev === 'critical') return 'Hostile forces mass on the borders of ' + loc + ' — invasion imminent!';
    if (sev === 'severe') return 'Serious military incidents on the border of ' + loc + '; readiness degrades.';
    if (sev === 'moderate') return 'Border incidents increase around ' + loc + '.';
...
export const EVENT_LIST: EventDefinition[] = Object.values(EVENT_REGISTRY);
...
export function evaluateAllTriggersForTerritory(ctx: ConditionContext): TriggerInfo[] {
  ...
  const wrapAndPush = (id: string, fn: () => { eligible: boolean; baseWeight: number; reasons: string[]; severityHint?: EventSeverity }) => {
    const s = fn();
    if (s.eligible) results.push({ eventId: id, weight: s.baseWeight, severityHint: s.severityHint, reasons: s.reasons });
  };

  // DROUGHT
  wrapAndPush(DROUGHT.id, () => buildTriggerFn({
    baseWeight: C.drought.baseWeight,
    eligibility: (cx) => COMMON_ELIGIBILITY.ownedTerritory(cx) && COMMON_ELIGIBILITY.noActiveDuplicate('drought')(cx),
    modifiers: [
  ...
  // FLOOD
  wrapAndPush(FLOOD.id, () => buildTriggerFn({
    baseWeight: C.flood.baseWeight,
    eligibility: cx => COMMON_ELIGIBILITY.ownedTerritory(cx) && COMMON_ELIGIBILITY.noActiveDuplicate('flood')(cx),
    modifiers: [
  ...
  // STORM
  wrapAndPush(STORM.id, () => buildTriggerFn({
    baseWeight: C.storm.baseWeight,
    eligibility: cx => COMMON_ELIGIBILITY.ownedTerritory(cx) && COMMON_ELIGIBILITY.noActiveDuplicate('storm')(cx),
    modifiers: [
  ...
  // HARSH WINTER
  wrapAndPush(HARSH_WINTER.id, () => buildTriggerFn({
    baseWeight: C.harshWinter.baseWeight,
    eligibility: cx => COMMON_ELIGIBILITY.ownedTerritory(cx) && COMMON_ELIGIBILITY.noActiveDuplicate('harsh_winter')(cx),
    modifiers: [
  ...
  // RESOURCE DISCOVERY
  wrapAndPush(RESOURCE_DISCOVERY.id, () => buildTriggerFn({
    baseWeight: C.resourceDiscovery.baseWeight,
    eligibility: cx => COMMON_ELIGIBILITY.ownedTerritory(cx) && COMMON_ELIGIBILITY.noActiveDuplicate('resource_discovery')(cx),
    modifiers: [
  ...
  // FOOD SHORTAGE
  wrapAndPush(FOOD_SHORTAGE.id, () => buildTriggerFn({
    baseWeight: C.foodShortage.baseWeight,
    eligibility: cx => COMMON_ELIGIBILITY.ownedTerritory(cx) && COMMON_ELIGIBILITY.noActiveDuplicate('food_shortage')(cx),
    modifiers: [
      ...
      { when: () => { const f = factionId ? ctx.helper.getFaction(factionId) : null; if (!f) return false; return ctx.helper.getFactionResourceRatio(f.id, 'food') < T.lowFoodRatio; }, apply: s => ({ ...s, baseWeight: s.baseWeight * C.foodShortage.lowFoodReserveBoost }), reason: () => 'Low food reserves' },
      { when: () => { const f = factionId ? ctx.helper.getFaction(factionId) : null; if (!f) return false; return ctx.helper.getFactionResourceRatio(f.id, 'food') < T.criticalFoodRatio; }, apply: s => ({ ...s, severityHint: 'severe' }), reason: () => 'Critical food ratio' },
    ],
  })(ctx));

  // FAMINE
  wrapAndPush(FAMINE.id, () => buildTriggerFn({
    baseWeight: C.famine.baseWeight,
    eligibility: cx => COMMON_ELIGIBILITY.ownedTerritory(cx) && COMMON_ELIGIBILITY.noActiveDuplicate('famine')(cx),
    modifiers: [
      ...
      { when: () => { const f = factionId ? ctx.helper.getFaction(factionId) : null; if (!f) return false; return ctx.helper.getFactionResourceRatio(f.id, 'food') < T.criticalFoodRatio; }, apply: s => ({ ...s, baseWeight: s.baseWeight * C.famine.criticalFoodBoost }), reason: () => 'Critical food reserves' },
    ],
    severityByConditions: [
      { when: () => { if (!t) return false; const f = factionId ? ctx.helper.getFaction(factionId) : null; return ctx.helper.hasActiveEventOn(t.id, 'food_shortage') && !!f && ctx.helper.getFactionResourceRatio(f.id, 'food') < T.criticalFoodRatio; }, severity: 'severe', reason: () => 'Shortage + critical food' },
    ],
  })(ctx));

  // PROSPERITY
  wrapAndPush(PROSPERITY.id, () => buildTriggerFn({
    baseWeight: C.prosperity.baseWeight,
    eligibility: cx => COMMON_ELIGIBILITY.ownedTerritory(cx) && COMMON_ELIGIBILITY.noActiveDuplicate('prosperity')(cx),
    modifiers: [
      ...
      { when: () => { const f = factionId ? ctx.helper.getFaction(factionId) : null; if (!f) return false; return ctx.helper.getFactionResourceRatio(f.id, 'gold') >= T.goodProsperityRatio; }, apply: s => ({ ...s, baseWeight: s.baseWeight * C.prosperity.goodTradeBoost }), reason: () => 'Treasury surplus' },
    ],
  })(ctx));

  // TRADE BOOM
  wrapAndPush(TRADE_BOOM.id, () => buildTriggerFn({
    baseWeight: C.tradeBoom.baseWeight,
    eligibility: cx => COMMON_ELIGIBILITY.ownedTerritory(cx) && COMMON_ELIGIBILITY.noActiveDuplicate('trade_boom')(cx),
    modifiers: [
  ...
  // UNREST
  wrapAndPush(UNREST.id, () => buildTriggerFn({
    baseWeight: C.unrest.baseWeight,
    eligibility: cx => COMMON_ELIGIBILITY.ownedTerritory(cx) && COMMON_ELIGIBILITY.noActiveDuplicate('unrest')(cx),
    modifiers: [
  ...
  // REBELLION
  wrapAndPush(REBELLION.id, () => buildTriggerFn({
    baseWeight: C.rebellion.baseWeight,
    eligibility: cx => COMMON_ELIGIBILITY.ownedTerritory(cx) && COMMON_ELIGIBILITY.noActiveDuplicate('rebellion')(cx),
    modifiers: [
    ...
    severityByConditions: [
      { when: () => { if (!t) return false; const f = factionId ? ctx.helper.getFaction(factionId) : null; return ctx.helper.hasActiveEventOn(t.id, 'unrest') && !!f && f.stability < T.criticalFactionStability; }, severity: 'severe', reason: () => 'Unrest + critical stability' },
    ],
  })(ctx));

  // BORDER TENSION
  wrapAndPush(BORDER_TENSION.id, () => buildTriggerFn({
    baseWeight: C.borderTension.baseWeight,
    eligibility: cx => COMMON_ELIGIBILITY.ownedTerritory(cx) && COMMON_ELIGIBILITY.noActiveDuplicate('border_tension')(cx),
    modifiers: [
