...
import { MemorySystem, MemorySummary } from '../memory/MemorySystem';
import { GoalSystem } from '../goals/GoalSystem';
import { SeededRNG } from '../utils/SeededRNG';

const { scoring: S, military: M, territory: T, diplomacy: D, economy: E, risk: R } = BALANCE;
...
export class ScoringHelpers {
  ...
  static computeTotalMilitaryPower(
    myArmies: Army[],
  ...
  static computeTerritoryMilitaryDefense(t: Territory, garrisonOnly = false): number {
  ...
  static evaluateTerritoryValue(t: Territory): number {
    const resourceVal = Object.entries(t.resourceOutput).reduce((s, [k, v]) => {
      const weight = (E.goldWeight as unknown as Record<string, number>)[`${k}Weight`] ?? 0.5;
  ...
  static evaluateResourceNeed(
    ...
    for (const k of keys) {
      ...
      scarcity[k] = sc;
      totalNeed += sc * ((E.goldWeight as unknown as Record<string, number>)[`${k}Weight`] ?? 0.5);
    }
...
export class ActionScorer {
  ...
  private baseScored(action: ActionType, input: ScorerInput): {
    base: number;
    factors: ScoredAction['factorBreakdown'];
    reasoning: string[];
  } {
    ...
    const factors: ScoredAction['factorBreakdown'] = [
  ...
  private scoreScout(input: ScorerInput): ScoredAction[] {
    ...
    for (const myT of ctx.myTerritories) {
      for (const nId of myT.neighboring) {
        const t = ctx.allTerritories.get(nId);
        if (t && !ctx.self.knownTerritories.includes(t.id)) candidates.push(t);
      }
      for (const nId of myT.neighboring) {
        const t = ctx.allTerritories.get(nId);
  ...
  private scoreMove(input: ScorerInput): ScoredAction[] {
    ...
    for (const army of ctx.myArmies) {
      ...
      for (const nId of currentLoc.neighboring) {
        ...
        if (!dest) continue;
        if (!ctx.self.territories.includes(dest.id) && !dest.neighboring.some((nn) => ctx.self.territories.includes(nn))) continue;
        let ms = 0;
  ...
  private scoreOfferPeace(input: ScorerInput): ScoredAction[] {
    ...
    for (const [targetId, rel] of atWarRelations) {
      ...
      if (ratio < 0.6) { score += 30; reasoning.push('losing war, seek peace'); }
      else if (ratio < 0.9) { score += 15; reasoning.push('stalemate, peace beneficial'); }
      else if (ratio > 1.3) { score -= 20; reasoning.push('winning, why offer peace?'); }
      ...
      if (mem.territoriesLost > mem.territoriesGained) {
        score += 10; reasoning.push('lost territory in this war');
      }

      if (rel.treaties.some((t) => t.type === 'alliance')) {
        score += 15; reasoning.push('former allies, possible reconciliation');
      }
  ...
  private scoreDeclareWar(input: ScorerInput): ScoredAction[] {
    ...
    for (const targetId of knownHostiles) {
      ...
      const hasBorder = ctx.myTerritories.some((t) =>
        t.neighboring.some((nId) => {
          const n = ctx.allTerritories.get(nId);
      ...
      const mem = memory.summarizeForFaction(targetId, turn);
      if (mem.totalGrievances > 25) { score += 20; reasoning.push('history of grievances demands war'); }
      if (mem.recentAttacks > 0) { score += 15; reasoning.push('recent attacks justify declaration'); }
  ...
  private scoreTrade(input: ScorerInput): ScoredAction[] {
    ...
    for (const targetId of tradePartners) {
      ...
      let matchScore = 0;
      let hasMatch = false;
      for (const k of Object.keys(scarcity) as (keyof Resources)[]) {
      ...
      if (rel?.treaties.some((t) => t.type === 'trade_agreement')) { score += 10; reasoning.push('trade agreement in place'); }

      const mem = memory.summarizeForFaction(targetId, turn);
      if (mem.treatiesBroken > 0) { score -= 15 * mem.treatiesBroken; reasoning.push('past unreliability'); }
      if (mem.successfulTrades > 0) { score += mem.successfulTrades * 3; factors.push({ factor: 'Trade history', weight: 1, contribution: mem.successfulTrades * 3 }); }
