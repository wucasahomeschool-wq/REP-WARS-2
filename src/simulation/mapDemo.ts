...
import { FactionId, InitialWorldParams, ExpansionRequest } from '../types';
...
export function runMapDemo(opts: {
  ...
  const params: InitialWorldParams = {
    worldSeed: seed,
  ...
  if (verbose) {
    ...
    for (const r of world.regions.values()) {
      ...
      if (theme) out.push(`         env=${theme.environmentalTag ?? 'n/a'}  strategic=${theme.strategicTendency}  rarity=${theme.rarity}`);
  ...
  for (const fid of factions) {
    ...
    const discovered = [] as string[];
    const scouted = [] as string[];
    const controlled = [] as string[];
    for (const [id, info] of v.visibility.entries()) {
      if (info.state === 'controlled') controlled.push(id);
      else if (info.state === 'scouted') scouted.push(id);
      else if (info.state === 'discovered') discovered.push(id);
  ...
  let sameWorld =
    world.territories.size === world2.territories.size &&
  ...
  for (const fid of factions) {
    ...
    if (world.territories.get(c1!)?.name !== world2.territories.get(c2!)?.name) capsSame = false;
  }
  ...
  const paramsAlt: InitialWorldParams = { ...params, worldSeed: 2026 };
  ...
  /* ── TEST 1: No isolated territories ────────────────────────── */
  {
    ...
    let isoCount = 0;
    for (const [, t] of w1.territories) if (t.neighboring.length === 0) isoCount++;
    tests.push({
  ...
  /* ── TEST 2: New territories connect to existing frontier ───── */
  {
    ...
    if (frontierIds.length > 0) {
      ...
      const connectsOk = res.newTerritories.every(nt =>
        nt.neighboring.some(nid => nid === pick || !res.newTerritories.some(x => x.id === nid))
      );
  ...
  /* ── TEST 4: Expansion does NOT overwrite existing territories ─ */
  {
    ...
    for (const [id, t] of w4.territories) snapshot.set(id, { name: t.name, terrain: t.terrain, owner: t.owner });
    ...
    for (let i = 0; i < Math.min(3, frontierIds.length); i++) {
      ...
      if (origIntact && res.validation.noOverwrites) expansionsOk++;
    }
    tests.push({
  ...
  /* ── TEST 5: Different seeds → different worlds ─────────────── */
  {
    ...
    let shared = 0;
    for (const n of namesA) if (namesB.has(n)) shared++;
  ...
  /* ── TEST 6: Same seed → identical world (determinism) ──────── */
  {
    ...
    const params6: InitialWorldParams = { worldSeed: SEED, playerFactionIds: ['a', 'b'] };
    ...
    for (const fid of ['a', 'b']) {
      ...
      if (wx.territories.get(c1!)?.name !== wy.territories.get(c2!)?.name) capitalsMatch = false;
    }
    tests.push({
  ...
  /* ── TEST 7: Scout revealTerritories works across range ─────── */
  {
    ...
    if (capitalId) {
      const beforeUnknown = Array.from(w7.territories.keys()).filter(
        id => (vis.visibility.get(id)?.state ?? 'unknown') === 'unknown'
      ).length;
      const res = engineT.revealTerritories(w7, capitalId, 3, vis, 'scout');
      const afterUnknown = Array.from(w7.territories.keys()).filter(
        id => (vis.visibility.get(id)?.state ?? 'unknown') === 'unknown'
      ).length;
      tests.push({
