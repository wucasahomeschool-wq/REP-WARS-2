...
import { Army, Territory, BattleInput, BattleResult, TerrainType } from '../types';
...
export function runBattleTestSuite(opts?: { seed?: number; verbose?: boolean }): BattleTestReport[] {
  const seed = opts?.seed ?? BALANCE.simulate.defaultSeed;
  const verbose = opts?.verbose ?? false;
  const engine = new BattleEngine();
  const reports: BattleTestReport[] = [];
...
export function runSeededReproducibilityTest(): { same: boolean; result1: BattleResult; result2: BattleResult; diffSeedResult: BattleResult } {
  const engine = new BattleEngine();
...
export function runStatisticalSample(
  inputFactory: (s: number) => BattleInput,
  runs: number = 200
): DistributionRun {
  const engine = new BattleEngine();
...
export function formatTestSuite(reports: BattleTestReport[]): string {
  const out: string[] = [];
  out.push('╔══════════════════════════════════════════════════════════════╗');
