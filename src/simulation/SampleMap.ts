...
export interface WarlordSpec {
  ...
  startingArmy: { soldiers: number; knights: number; siege: number };
  startingResources: Resources;
  startingIncome: Partial<Resources>;
  relationships: { target: string; state: RelationshipState; opinion: number }[];
}
...
export class SimulationBuilder {
  static buildFromSpecs(
    mapSpecs: MapTerritorySpec[],
    warlordSpecs: WarlordSpec[],
    seed: number = BALANCE.simulate.defaultSeed
  ): {
    ...
    for (const spec of warlordSpecs) {
      const personality = PersonalitySystem.randomizePreset(
        spec.personality,
        spec.personalityVariant,
        rng
      );
      ...
      if (spec.startingArmy.soldiers + spec.startingArmy.knights > 0 && spec.startingTerritories.length > 0) {
        ...
        const capitalT = spec.startingTerritories.find(
          (tid) => territories.get(tid)?.isCapital
        ) ?? spec.startingTerritories[0];
      ...
      const goals = GoalSystem.generateInitialGoals(
        spec.personality,
        spec.id,
        0,
        rng
      ...
      for (const tid of spec.startingTerritories) {
        ...
        if (t) for (const n of t.neighboring) knownTerritories.add(n);
      }

      const knownFactions = new Set<string>([spec.id]);
      for (const tid of knownTerritories) {
        const t = territories.get(tid);
        if (t?.owner) knownFactions.add(t.owner);
      }
      for (const rel of spec.relationships) knownFactions.add(rel.target);
    ...
    for (const [id, snap] of factions) {
      ...
      for (const t of ctx.myTerritories) totalPower += t.garrison * BALANCE.military.soldierValue;
