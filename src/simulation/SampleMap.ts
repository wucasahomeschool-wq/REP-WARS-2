import { Territory, GameStateSnapshot, RelationshipState, PersonalityType, Resources, WarlordSnapshot, Army, DiplomaticRelationship, MapTerritorySpec } from '../types';
...
// `MapTerritorySpec` is now canonical in `../types` (also used by
// `MapEngine.toTerritorySpecs`). Re-exported here for backwards
// compatibility since `src/index.ts` re-exports it from this module.
export type { MapTerritorySpec } from '../types';
...
export interface WarlordSpec {
  ...
  /** Optional; defaults to `BALANCE.ambition.defaultValue` (0.5). Not a personality trait. */
  ambition?: number;
...
export class SimulationBuilder {
  static buildFromSpecs(mapSpecs: MapTerritorySpec[], warlordSpecs: WarlordSpec[], seed = BALANCE.simulate.defaultSeed): {
    ...
    const warlordStates = new Map<string, WarlordState>();
    let armyIdCounter = 0;
    ...
    for (const spec of warlordSpecs) {
      const personality = PersonalitySystem.randomizePreset(spec.personality, spec.personalityVariant, rng);
      ...
      const rivalFactionIds = spec.relationships
        .filter((r) => r.state === 'hostile' || r.state === 'at_war' || r.state === 'tense' || r.opinion < -30)
        .map((r) => r.target)
        .filter((id) => factionIds.includes(id));
      const allianceCandidateIds = spec.relationships
        .filter((r) => r.state === 'allied' || r.state === 'friendly' || r.state === 'neutral')
        .map((r) => r.target)
        .filter((id) => factionIds.includes(id) && id !== spec.id);
      const goals = GoalSystem.generateInitialGoals(spec.personality, spec.id, 0, rng, {
      ...
      const snapshot: WarlordSnapshot = {
        ...
        name: spec.name,
        personality,
        ambition: spec.ambition ?? BALANCE.ambition.defaultValue,
      ...
      const ws = new WarlordState(snapshot, new MemorySystem(), new GoalSystem(snapshot.goals));
    ...
    for (const [id, snap] of factions) {
      ...
      for (const aid of snap.armies) {
        ...
        if (a) {
          totalPower += a.soldiers * BALANCE.military.soldierValue
            + a.knights * BALANCE.military.knightValue
            + a.siegeEngines * BALANCE.military.siegeValue;
        }
      }
      for (const t of ctx.myTerritories)
        totalPower += t.garrison * BALANCE.military.soldierValue;
