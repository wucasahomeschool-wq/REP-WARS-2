...
import { runBattleTestSuite, formatTestSuite, runSeededReproducibilityTest } from './battleTests';
import { runMapDemo } from './mapDemo';
...
function parseArgs(argv: string[]): SimOptions {
  ...
  for (const arg of argv) {
    ...
    else if (arg === '--battles' || arg === '-b') opts.battles = true;
    else if (arg === '--battle-repro') opts.battleRepro = true;
    else if (arg === '--map' || arg === '-m') opts.map = true;
    else if (arg.startsWith('--map-iterations=')) opts.mapIterations = Math.max(1, parseInt(arg.split('=')[1]) || opts.mapIterations);
...
async function runBattleMode(opts: SimOptions) {
  printBanner();
  if (opts.battleRepro) {
...
async function runMapMode(opts: SimOptions) {
