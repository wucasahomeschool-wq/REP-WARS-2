...
export interface GoalAlignmentResult {
  ...
  relevantGoals: { goal: StrategicGoal; alignment: 'aligned' | 'misaligned' | 'neutral' }[];
}
...
export class GoalSystem {
  ...
  getActiveGoals(currentTurn: number): StrategicGoal[] {
    return this.goals.filter((g) => {
      if (g.progress >= g.targetProgress) return false;
      if (g.deadlineTurn !== null && currentTurn > g.deadlineTurn) return false;
      return true;
  ...
  evaluateActionAlignment(params: {
    ...
    const relevant: { goal: StrategicGoal; alignment: 'aligned' | 'misaligned' | 'neutral' }[] = [];
    ...
    switch (goal.type) {
      case 'destroy_rival':
        if (goal.targetFaction) {
          if (targetFaction === goal.targetFaction) {
            if (actionType === 'ATTACK' || actionType === 'DECLARE_WAR') return 'aligned';
            if (actionType === 'OFFER_PEACE' || actionType === 'NEGOTIATE') return 'misaligned';
          }
          if (actionType === 'TRADE' && targetFaction === goal.targetFaction) return 'misaligned';
        }
        break;
      ...
      case 'control_region':
        if (targetTerritory && goal.targetRegion) {
          ...
          if (actionType === 'ATTACK' && terrRegion === goal.targetRegion) return 'aligned';
          if (actionType === 'EXPAND' && terrRegion === goal.targetRegion) return 'aligned';
        }
        if (actionType === 'BUILD' && targetTerritory) return 'aligned';
        break;

      case 'protect_territory':
        if (goal.targetTerritory === targetTerritory) {
          if (actionType === 'DEFEND' || actionType === 'REINFORCE' || actionType === 'BUILD') return 'aligned';
          if (actionType === 'RETREAT') return 'misaligned';
        }
      ...
      case 'dominant_faction':
        if (actionType === 'ATTACK' || actionType === 'EXPAND' || actionType === 'DECLARE_WAR') return 'aligned';
        if (actionType === 'WAIT' || actionType === 'OFFER_PEACE') return 'misaligned';
        break;

      case 'prepare_for_invasion':
        if (actionType === 'BUILD' || actionType === 'REINFORCE' || actionType === 'DEFEND') return 'aligned';
        if (actionType === 'ATTACK') return 'aligned';
        if (actionType === 'WAIT') return 'misaligned';
        break;

      case 'economic_growth':
        if (actionType === 'BUILD' || actionType === 'TRADE' || actionType === 'SCOUT') return 'aligned';
        if (actionType === 'DECLARE_WAR' || actionType === 'ATTACK') return 'misaligned';
        break;

      case 'form_alliance':
        if (goal.targetFaction) {
          if (targetFaction === goal.targetFaction) {
            if (actionType === 'NEGOTIATE' || actionType === 'TRADE') return 'aligned';
            if (actionType === 'DECLARE_WAR' || actionType === 'ATTACK') return 'misaligned';
          }
        }
        break;
      ...
      case 'break_siege':
        if (actionType === 'ATTACK' || actionType === 'REINFORCE' || actionType === 'MOVE') return 'aligned';
        if (actionType === 'RETREAT') return 'misaligned';
        break;
    }
