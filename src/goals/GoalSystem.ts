 ...
 * scoring. Does not invent region ids — `control_region` still cannot
 * align via region membership because `Territory` has no `regionId`.
 */
...
export class GoalSystem {
   ...
   * `control_region` with a null region is a known limitation, not invalid.
   */
  ...
  private checkAlignment(goal: StrategicGoal, params: AlignmentParams): 'aligned' | 'misaligned' | 'neutral' {
    ...
    switch (goal.type) {
      ...
      case 'destroy_rival':
        ...
        break;
      // `control_region` bug fix: this used to also gate on `goal.targetRegion`
      // (always null today, for the same reason as above — never set by
      // `generateInitialGoals`) and, even when reached, derived a "region"
      // via `terr.id.split('_')[0]`. That is the exact bug reported for
      // this pass: MapEngine-generated ids like `t_23_mis` split to `t`,
      // and hand-authored SAMPLE_MAP ids like `north_valley` split to the
      // arbitrary token `north` — neither is a real region identifier.
      // There is in fact no region concept on the canonical `Territory`
      // type at all (region membership only exists one layer up, in
      // `MapWorldState.regions`, which `GoalSystem`/`ActionContext` never
      // receive) so this could not be made correct without adding a
      // `Territory.regionId`-equivalent field — an actual data-model
      // change, out of scope for a bug-fix pass. The broken id-parsing
      // branch is removed; region-based alignment is deferred and
      // documented rather than faked. The unconditional BUILD fallback
      // below (fortify wherever the goal targets) is unchanged and is the
      // only currently-functional part of `control_region`.
      case 'control_region':
