...
import { TERRAIN_NAMES } from '../constants/balance';
...
export class MapEngine {
  ...
  private pickTheme(
    ...
    for (const t of pool) {
      ...
      if (hint && t.id === hint) s *= 3;
      for (const adjId of adjacentThemes) {
        if (adjId === t.id) s *= this.C.themes.sameThemeBiasWhenAdjacent;
        if (t.allowedAdjacentThemes.includes(adjId)) s *= 1.4;
      }
      if (preferUnused && preferUnused.includes(t.id)) s *= 2;
      scores.set(t.id, s);
    }
  ...
  private fabricateTerritory(
    ...
    const terrain: TerrainType = isCapital && theme.preferredTerrain[0]
    ...
    for (const tierStr of ['0', '1', '2', '3', '4', '5']) {
      fortSum += fw[tierStr as any] ?? 0;
      if (fortRoll < fortSum) { fortTier = parseInt(tierStr); break; }
    }
    if (opts.fortHint !== undefined) fortTier = opts.fortHint;
    if (isCapital) fortTier = Math.max(fortTier, Cg.fortificationCapitalDefault);
    else if (opts.owner && !isCapital) fortTier = Math.max(fortTier, Cg.fortificationStartDefault);
    ...
    if (isCapital) baseValue = Math.max(baseValue, rng.nextInt(Cg.capitalBaseValueMin, Cg.capitalBaseValueMax));
    if (opts.baseValueMult) baseValue = Math.round(baseValue * opts.baseValueMult);
    ...
    for (let r = 0; r < resCount; r++) {
      ...
      if (!resourceOutput[res]) resourceOutput[res] = 0;
    ...
    const terr: Territory = {
      ...
      baseValue,
      resourceOutput,
      fortification: fortTier as FortificationTier,
  ...
  private bidirConnect(a: Territory, b: Territory): void {
    if (a.id === b.id) return;
    if (!a.neighboring.includes(b.id)) a.neighboring.push(b.id);
    if (!b.neighboring.includes(a.id)) b.neighboring.push(a.id);
  }
  ...
  private computeNeighborsFor(world: MapWorldState, id: TerritoryId, rng: SeededRNG): void {
    ...
    direct.forEach(nid => {
      if (nid !== id) this.bidirConnect(t, world.territories.get(nid)!);
    });
    for (const d of HEX_DIAG) {
      ...
      const sharedAdj = HEX_DIRS.filter(dd => {
        ...
        if (!midA) return false;
        return hexDist(tpos, { q: pos.q + dd.q, r: pos.r + dd.r }) < 1.5;
      });
      if (sharedAdj.length === 0) continue;
    ...
    if (t.neighboring.length < this.C.territory.minNeighbors) {
      ...
      for (const [otherId, otherPos] of world.graphMeta.territoryPos.entries()) {
        if (otherId === id || t.neighboring.includes(otherId)) continue;
      ...
      if (best) this.bidirConnect(t, world.territories.get(best.id)!);
    }
  }
  ...
  private refreshFrontier(world: MapWorldState): void {
    ...
    for (const [id, terr] of world.territories.entries()) {
      ...
      for (const d of HEX_DIRS) {
        ...
        if (!neighborId) { frontier.add(id); break; }
      }
    }
  ...
  private ensureNeighborsSane(world: MapWorldState): void {
    for (const [id, terr] of world.territories.entries()) {
      ...
      for (const nid of neigh) {
        ...
        if (n && !n.neighboring.includes(id)) n.neighboring.push(id);
      }
      ...
      if (neigh.length < this.C.territory.minNeighbors && world.territories.size > 2) {
        ...
        if (pos) {
          let best: { id: TerritoryId; dist: number } | null = null;
          for (const [otherId, otherPos] of world.graphMeta.territoryPos.entries()) {
            if (otherId === id || neigh.includes(otherId)) continue;
            const d = hexDist(pos, otherPos);
            if (!best || d < best.dist) best = { id: otherId, dist: d };
          }
          if (best) this.bidirConnect(terr, world.territories.get(best.id)!);
        }
      }
    }
  }
  ...
  public createEmptyWorld(worldSeed: number, themes?: ThemeDefinition[]): MapWorldState {
    const themeList = themes ?? DEFAULT_THEME_LIBRARY;
    const themeMap = new Map<ThemeId, ThemeDefinition>();
    for (const t of themeList) themeMap.set(t.id, t);
    return {
      worldSeed,
      turn: 1,
      territories: new Map(),
  ...
  public generateInitialWorld(params: InitialWorldParams): { world: MapWorldState; playerCapitals: Map<FactionId, TerritoryId>; visibility: Map<FactionId, PlayerVisibilityMap> } {
    ...
    for (let i = 0; i < targetRegions; i++) centerThemes.push(this.pickTheme(rng, themeArr, []));

    const regionRadii = [1, 2, 1, 2, 1, 1].slice(0, Math.max(1, targetRegions));
    let regionCenters: { pos: Coord; theme: ThemeDefinition; radius: number }[] = [];
    ...
    while (placed < targetRegions && attempts < C.maxStartSearchAttempts * 10) {
      ...
      const pos: Coord = { q: d.q * 3 * (ring + 1), r: d.r * 3 * (ring + 1) };
      if (regionCenters.some(c => hexDist(c.pos, pos) < 3)) continue;
    ...
    if (regionCenters.length === 0) regionCenters.push({ pos: { q: 0, r: 0 }, theme: centerThemes[0], radius: 2 });
    ...
    for (const rc of regionCenters) {
      ...
      const fillPositions: Coord[] = [{ q: rc.pos.q, r: rc.pos.r }];
      for (let ring = 1; ring < ringCount; ring++) {
        let pos: Coord = { q: rc.pos.q + HEX_DIRS[4].q * ring, r: rc.pos.r + HEX_DIRS[4].r * ring };
      ...
      for (const fp of fillPositions) {
        ...
        if (usedSpots.has(nkey)) continue;
        if (rrng.next() < pert && ringCount > 1) continue;
        usedSpots.add(nkey);
        filtered.push(fp);
      }
      ...
      for (let i = 0; i < filtered.length; i++) {
        ...
        if (i === 0) centerId = terr.id;
      }
      region.centerTerritoryId = centerId;
      world.regions.set(rid, region);
    }

    for (const id of Array.from(world.territories.keys())) this.computeNeighborsFor(world, id, rng);
    ...
    if (playerCount > 0) {
      ...
      for (const cand of shuffled) {
        if (capitals.length >= playerCount) break;
      ...
      for (let i = 0; i < playerCount; i++) {
        ...
        if (region && !region.isCapitalRegion) { region.isCapitalRegion = true; }
        capT.owner = fid;
        capT.isCapital = true;
        capT.fortification = Math.max(capT.fortification, this.C.territory.fortificationCapitalDefault as any);
        ...
        for (const [tid, t] of withinRadius) {
          if (granted >= perFactionStart * 2 + 1) break;
          if (!t.owner || t.owner === fid) {
            t.owner = fid;
            t.isKnown = true;
            if (!t.isCapital) {
              t.fortification = Math.max(t.fortification, 1 as any);
      ...
      for (const t of world.territories.values()) t.isKnown = true;
    }
  ...
  /* ─── Expansion ──────────────────────────────────────────────────── */
  public expandFromFrontier(req: ExpansionRequest): ExpansionResult {
    ...
    if (!fromT) { result.validation.allConnected = false; return result; }
    ...
    const existingAdjacentThemes: ThemeId[] = fromT.neighboring
    ...
    for (let attempt = 0; attempt < Cg.frontierConnectionRetries && remaining > 0; attempt++) {
      ...
      if (world.graphMeta.coordToTerritory.has(k)) continue;
      ...
      if (currentRegion) {
        ...
        if (!currentRegion.centerTerritoryId) currentRegion.centerTerritoryId = newT.id;
      }
      ...
      for (const nid of newT.neighboring) {
        ...
        if (nid !== req.fromFrontierTerritoryId) result.newlyAdjacentExistingTerritories.push(nid);
      }
      result.newConnections.push({ from: req.fromFrontierTerritoryId, to: newT.id });
    }

    for (const t of result.newTerritories) this.computeNeighborsFor(world, t.id, rng);
    ...
    result.validation.allConnected = !result.newTerritories.some(t =>
      t.neighboring.length > 0 &&
  ...
  public regionFor(world: MapWorldState, tid: TerritoryId): RegionId | null {
    for (const r of world.regions.values()) if (r.territories.includes(tid)) return r.id;
    return null;
  }
  public regionForObj(world: MapWorldState, tid: TerritoryId): Region | null {
    for (const r of world.regions.values()) if (r.territories.includes(tid)) return r;
    return null;
  }
  public themeFor(world: MapWorldState, tid: TerritoryId): ThemeId | null {
  ...
  public revealNeighbors(
    ...
    for (const [id, terr] of world.territories.entries()) {
      ...
      const prevState: VisibilityState = prev?.state ?? 'unknown';
      ...
      if (rank(desired) > rank(prevState)) {
        ...
        if (prevState === 'unknown') newly.push(id);
      ...
      if (region) visibility.knownRegions.add(region);
      const theme = this.themeFor(world, id);
      if (theme) visibility.knownThemes.add(theme);
    }
    return { newly, advanced };
  }

  public recomputeVisibilityFor(world: MapWorldState, faction: FactionId, visibility: PlayerVisibilityMap): void {
  ...
  public createVisibilityMapFor(world: MapWorldState, faction: FactionId): PlayerVisibilityMap {
  ...
  public revealTerritories(
    ...
    for (const [tid, v] of visibility.visibility.entries()) beforeByT.set(tid, v.state);
    const startT = world.territories.get(origin);
    if (startT) this.revealNeighbors(world, origin, visibility, range, revealedBy);
    ...
    for (const [tid, v] of visibility.visibility.entries()) {
      ...
      if (before === 'unknown' && (v.state === 'discovered' || v.state === 'scouted' || v.state === 'controlled')) res.newlyDiscovered.push(tid);
      if ((before === 'unknown' || before === 'discovered') && v.state === 'scouted') res.newlyScouted.push(tid);
    }
    return res;
  }

  /* ─── Reports & rendering helpers ────────────────────────────────── */
  public generateReport(world: MapWorldState, capitals?: Map<FactionId, TerritoryId>): MapGenerationReport {
    ...
    for (const t of world.territories.values()) {
      ...
      if (r) themeDist[r.themeId] = (themeDist[r.themeId] ?? 0) + 1;
    }
  ...
  public renderWorldText(world: MapWorldState, visibility?: PlayerVisibilityMap, faction?: FactionId): string {
    const lines: string[] = [];
    ...
    const grid: string[][] = [];
    for (let r = 0; r < height * 2 + 1; r++) grid.push(new Array(width * 4 + 2).fill(' '));
    for (const [id, t] of world.territories.entries()) {
      ...
      if (visibility) {
        ...
        if (!v) stateLabel = '·';
        else if (v.state === 'controlled') stateLabel = '#';
        else if (v.state === 'scouted') stateLabel = '!';
        else if (v.state === 'discovered') stateLabel = 'o';
        else stateLabel = '·';
      } else {
      ...
      for (let i = 0; i < display.length; i++) grid[row][col + i] = display[i];
      const tlabel = visibility && !visibility.visibility.has(id) ? '      ' : `${id.split('_')[1].padStart(3, '0')}${t.isCapital ? 'K' : '.'}${stateLabel}`;
      if (row + 1 < grid.length) for (let i = 0; i < tlabel.length; i++) grid[row + 1][col + i] = tlabel[i];
    }
    for (let r = 0; r < grid.length; r++) {
      const buf: string[] = [];
      let hasChar = false;
      for (let c = 0; c < grid[r].length; c++) { if (grid[r][c] !== ' ') hasChar = true; buf.push(grid[r][c]); }
      if (hasChar) lines.push(buf.join(''));
    }
    return lines.join('\n');
  }

  public renderTerritoryTable(world: MapWorldState, visibility?: PlayerVisibilityMap): string {
    const lines: string[] = [];
    lines.push('┌─────┬──────────────────────────┬────────────┬──────┬──────────┬─────┬──────┬────────┬────────┐');
  ...
  public validateWorld(world: MapWorldState): {
    noIsolated: boolean;
    ...
    for (const [id, t] of world.territories) if (t.neighboring.length === 0) isolated.push(id);
    let allBidir = true;
    for (const [id, t] of world.territories) {
      for (const nid of t.neighboring) {
        const n = world.territories.get(nid);
        if (!n || !n.neighboring.includes(id)) { allBidir = false; }
      }
    }
    let covered = 0;
    for (const r of world.regions.values()) covered += r.territories.length;
    const names = new Set<string>();
    let dup = false;
    for (const t of world.territories.values()) {
      if (names.has(t.name)) dup = true;
      names.add(t.name);
    }
    return {
  ...
  public toTerritorySpecs(world: MapWorldState): Array<{
    id: string;
    name: string;
  ...
  public collectFactionTerritoryIds(world: MapWorldState, factionId: FactionId): TerritoryId[] {
    const ids: TerritoryId[] = [];
    for (const [id, t] of world.territories) {
      if (t.owner === factionId) ids.push(id);
    }
    return ids;
  }

  public collectAdjacentFrontiers(world: MapWorldState, factionId: FactionId): TerritoryId[] {
    ...
    for (const ownedId of owned) {
      ...
      if (!t) continue;
      for (const nid of t.neighboring) {
        if (!owned.has(nid)) frontiers.add(nid);
      }
      if (world.graphMeta.frontierTerritories.has(ownedId)) frontiers.add(ownedId);
    }
